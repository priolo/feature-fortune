...
class FundingRoute extends httpRouter.Service {
	...
	private async paymentAllExpired() {
		const now = new Date()
		const expiredFundings: FundingRepo[] = await new Bus(this, this.state.repository).dispatch({
		...
		for (const funding of expiredFundings) {
			this.paymentFunding ( funding.id )
		}
	}
	 ...
	 */
	async paymentFunding(fundingId: string) {


		const funding = await new Bus(this, this.state.repository).dispatch({
			type: typeorm.Actions.FIND_ONE,
			payload: <FindManyOptions<FundingRepo>>{ 
				where: {id: fundingId},
				relations: { feature: { author: true }, account: true }
			}
		})
		...
		// get FEATURE and authorId
		const feature: any = await new Bus(this, "/typeorm/features").dispatch({
			type: typeorm.RepoRestActions.GET_BY_ID,
			payload: funding.featureId,
		})
